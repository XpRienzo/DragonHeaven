...
evgutter: function (target, room, user) {
	...
	typeoptspeed: function (target, room, user) {
		...
		Object.values(feDex).forEach(mon => {
			...
			let speedtierzero = 2 * mon.baseStats.spe + 36;
			let nameo = mon;
			buf += `${speedtierplusscarf}: Scarf Fast+ ${nameo}<br>${speedtierscarf}: Scarf Fast ${mon.species}<br>${speedtierplus}: Fast+ ${mon.species}<br>${speedtier}: Fast ${mon.species}<br>${speedtierzero}: Bulky ${mon.species}<br>`;
		});
		this.sendReplyBox(`${buf}</div>`);
	},
